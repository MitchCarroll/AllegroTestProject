/* C. Mitch Carroll
   ...
   I have yet to figure out some more advanced topics, like Android deployment.
...
//Allegro requires your main function to have this signature
{
  ...
  al_set_new_display_flags(ALLEGRO_WINDOWED); //do this stuff before creating
                                              //the new window
  ...
  //NOTE: DATADIR is a constant which is set by our CMake setup at compile time
  //it is a string that is tacked onto the front of the filename so the program
  //can find it (after installing)
  //e.g.: "/usr/local/share/AllegroTestProgram/data/"
  //this way, instead of typing someting like load("myFile.dat") you can do:
  //load(DATADIR "myFile.dat")
  al_clear_to_color(al_map_rgb_f(.5,.5,.5));
  if(!my_pic) //if the file doesn't load right, or isn't found
    cout << DATADIR << "test.png" << " DIDN'T LOAD!" << endl; //print an error
  else {
    al_draw_bitmap(my_pic, 0, 0, 0); //draw the image to the window
    al_draw_scaled_rotated_bitmap(my_pic, 256,256, 600,200, .5,1.5, 45, 0);
  }

  al_flip_display(); //flip the backbuffer to the screen so we can see it
      ...
      if(ev.type == ALLEGRO_EVENT_KEY_DOWN) //if it's a keypress...
	{
	  switch(ev.keyboard.keycode)
	    ...
	    case ALLEGRO_KEY_ESCAPE: //and it's the ESC key...
	      ...
	      break;
	    default: //it's good practice to always define a default case
	      break;
	    }
	}
    }
